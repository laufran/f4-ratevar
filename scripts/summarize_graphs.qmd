...
```{julia}
examplerep = DataFrame(CSV.File("output/g4hsearch1_examplerep.csv"))
...
g4match_newicks = h4_hwmatches %>%
  ...
  summarize(hash   = first(hash),
            newick = first(newick))
write.csv(g4match_newicks, "output/g4match_newicks.csv", row.names = FALSE)
```

```{julia}
g4match_newicks = DataFrame(CSV.File("output/g4match_newicks.csv"))
