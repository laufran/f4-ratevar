...
As of 10-16-2024, all 120 parameter sets' (for `g1` and `g4`) graph searches ran across all the Franklins/Darwins 
have been compiled back to individual Franklin machines. As in, their `graphreps` folders with 
at least 150 (more than 150 if `fleg-pruned` (g1) and `h_search` 3+ ran) individual graph search files
...
scp lefrankel@franklin03.stat.wisc.edu:/nobackup2/lefrankel/f4-ratevar/output/concat_topgraphs_all03.csv .
```
...
looks for estimated graphs (`truenet == FALSE`) when using `g1`
...
```{r}
net_matches = concat_topgraphs %>% 
  group_by(irep, paramid, hash, h_search)  %>%
  #search for groups that have matches in the hash
  #only keep groups that have more than 1
  filter(n() > 1) %>%
  #then filter that the truenet col has unique > 2
  #that way get hash matches between true & estimated graphs
  filter(length(unique(truenet)) == 2)
...
# Filter out duplicate graphs, only keep graphs within 10 pts of best scoring within each replicate (across run)
```{r}
...
## Add column `ishashmatch`

Hard code the hashes for the two true nets (hashes obtained above) and filter accordingly:
...
## Look at hard-wired distances for net matches
...
get graphs with hard-wired distance of 0 for `g4` (more inclusive than `ishashmatch == TRUE`):
...
## Load in summary csvs locally

*Note:*
From here on out, this code is assumed to be run locally.

```{r}
h1_byparamid = read.csv("output/h1_byparamid_filtered.csv")
h4_byparamid = read.csv("output/h4_byparamid_filtered.csv")
h1_byrep = read.csv("output/h1_byrep_filtered.csv")
h4_byrep = read.csv("output/h4_byrep_filtered.csv")
h4_hwmatches = read.csv("output/g4_hwmatches.csv")
...
plot_accuracy_2panels = function(dat, yvar, xlab = "search h", ylab, xlabels= waiver(), seed){
  ggplot(dat, aes(.data[["h_search"]], .data[[yvar]])) +
  geom_point(aes(color=lindist, shape=nind, fill=net),
  ...
  labs(subtitle="mutation rate per generation") +
...
let's split this into two figures:
```{r}
bothnets_trueh_manysnps = bothnets_trueh %>%
  filter(nbiallelic == 100000)

bothnets_trueh_fewsnps = bothnets_trueh %>%
  filter(nbiallelic == 10000)

plot_accuracy_2panels(bothnets_trueh_manysnps,  yvar = "prop_min_hashmatch",
                xlab = "network / search h",
                xlabels = isomorphism_labeller,
                ylab="% replicates with the true network in its set of top graphs", 
                seed=27)
ggsave("figures/isomorphism/fig_prophashmatches_manysnps.pdf", height=5, width=9) 

plot_accuracy_2panels(bothnets_trueh_fewsnps,  yvar = "prop_min_hashmatch",
                xlab = "network / search h",
                xlabels = isomorphism_labeller,
                ylab="% replicates with the true network in its set of top graphs", 
                seed=27)
ggsave("figures/isomorphism/fig_prophashmatches_fewsnps.pdf", height=5, width=9) 
```
...
## G1 h_search 1 vs. 2 on franklin00

So this will just be for G1 graphs:
15 of each on each cluster (franklin00-franklin03).
This is run on franklin00 after all individual csvs 
from other franklins were moved there.

```{r}
wr0 = read_csv("output/worstresid/fleg-pruned/worstresid0.csv")
wr1 = read_csv("output/worstresid/fleg-pruned/worstresid1.csv")
wr2 = read_csv("output/worstresid/fleg-pruned/worstresid2.csv")
wr3 = read_csv("output/worstresid/fleg-pruned/worstresid3.csv")
...
wr = wr %>% rowwise() %>% 
                      mutate(paramid = paste(c(nind, lindist, nbiallelic, mutrate, net), collapse="-"))
write.csv(wr, file = "output/worstresid/fleg-pruned/wr_all.csv", row.names = FALSE)
```
...
write.csv(type1_fromwr, "output/worstresid/fleg-pruned/type1_fromwr.csv", row.names = FALSE)

min(wr_byirep$ngraphs_h2_after) #1
#there is always at least 1 graph accepted from h_search==2
```

## G4 h_search 4 on franklin01

```{r}
wr0 = read_csv("output/worstresid/fleg/g4_worstresid0.csv")
wr1 = read_csv("output/worstresid/fleg/g4_worstresid1.csv")
wr2 = read_csv("output/worstresid/fleg/g4_worstresid2.csv")
wr3 = read_csv("output/worstresid/fleg/g4_worstresid3.csv")
...
wr = wr %>% rowwise() %>% 
                      mutate(paramid = paste(c(nind, lindist, nbiallelic, mutrate, net), collapse="-"))
write.csv(wr, file = "output/worstresid/fleg/wr_all.csv", row.names = FALSE)
...
write.csv(type1_fromwr, "output/worstresid/fleg/type1_fromwr.csv", row.names = FALSE)
```

## Visualize results locally

after downloading files locally, visualize:

```{r}
type1_wr = read.csv("output/h1_type1_fromwr.csv")
...
plot_accuracy_2panels = function(dat, ylab, seed){
...
plot_accuracy_2panels(type1_wr, ylab="proportion incorrectly rejecting h=1", seed=35)
ggsave("figures/worstresid/fig_g1_wrtype1.pdf", height=5, width=9)
```


```{r}
g4_type1_wr = read.csv("output/h4_type1_fromwr.csv")
...
plot_accuracy_2panels(g4_type1_wr, ylab="proportion incorrectly rejecting h=4", seed=35)
ggsave("figures/worstresid/fig_g4_wrtype1.pdf", height=5, width=9)
```